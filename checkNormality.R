...
parameters <- c("Signbased.vis", "Superlogical")
...
for(i in 1:2)
{
  ...
  {
    # Set filenames
    input <- paste("normalityInput\\", i, "_", parameters[p], ".csv", sep = "")
    outQQ <- paste("normalityOutput\\", i, "_", parameters[p], "-QQ.pdf", sep = "")
    outHist <- paste("normalityOutput\\", i, "_", parameters[p], "-Hist.pdf", sep = "")
    outStat <- paste("normalityOutput\\", i, "_", parameters[p], "-Stat.txt", sep = "")
